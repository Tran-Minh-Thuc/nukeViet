    ...
    foreach ($array_catpage as $array_row_i) {
        ...
        if ($a == 1) {
            if (defined('NV_IS_MODADMIN')) {
                $xtpl->assign('ADMINLINK', nv_link_edit_page($array_row_i['id']) . ' ' . nv_link_delete_page($array_row_i['id']) . ' ' . nv_link_export_pdf_page($array_row_i['id']));
    ...
    if (!empty($array_catcontent)) {
        ...
        foreach ($array_catcontent as $array_catcontent_i) {
            ...
            if ($a == 0) {
                ...
                if (defined('NV_IS_MODADMIN')) {
                    $xtpl->assign('ADMINLINK', nv_link_edit_page($array_catcontent_i['id']) . ' ' . nv_link_delete_page($array_catcontent_i['id']) . ' ' . nv_link_export_pdf_page($array_catcontent_i['id']));
    ...
    if (!empty($array_content)) {
        foreach ($array_content as $key => $array_content_i) {
            ...
            if ($key == 0) {
                ...
                if (defined('NV_IS_MODADMIN')) {
                    $xtpl->assign('ADMINLINK', nv_link_edit_page($array_content_i['id']) . ' ' . nv_link_delete_page($array_content_i['id']) . ' ' . nv_link_export_pdf_page($array_content_i['id']));
...
function detail_theme($news_contents, $array_keyword, $related_new_array, $related_array, $topic_array, $content_comment)
{
    ...
    if (defined('NV_IS_MODADMIN')) {
        $xtpl->assign('ADMINLINK', nv_link_edit_page($news_contents['id']) . ' ' . nv_link_delete_page($news_contents['id'] . ' ' . nv_link_export_pdf_page($news_contents['id']), 1));
    ...
    if (!empty($topic_array)) {
        foreach ($topic_array as $topic_array_i) {
            ...
            if ($topicid and defined('NV_IS_MODADMIN')) {
                $xtpl->assign('ADMINLINK', nv_link_edit_page($topic_array_i['id']) . ' ' . nv_link_delete_page($topic_array_i['id']) . ' ' . nv_link_export_pdf_page($topic_array_i['id']));
