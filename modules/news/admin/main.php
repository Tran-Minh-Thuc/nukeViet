...
foreach ($global_array_cat as $catid_i => $array_value) {
    ...
    if (defined('NV_IS_ADMIN_MODULE')) {
        ...
        if ($_cat_admin_i['admin'] == 1) {
            ...
            $check_cat = true;
        } elseif ($_cat_admin_i['export_pdf_content'] == 1) {
            $check_cat = true;
        }
    }
...
if (($module_config[$module_name]['elas_use'] == 1) and $checkss == NV_CHECK_SESSION) {
    ...
    foreach ($response['hits']['hits'] as $key => $value) {
        ...
        $check_permission_edit = $check_permission_delete = $check_permission_export_pdf = false;

        if (defined('NV_IS_ADMIN_MODULE')) {
            $check_permission_edit = $check_permission_delete = $check_permission_export_pdf = true;
        } else {
            $array_temp = explode(',', $listcatid);
            $check_edit = $check_del = $check_export_pdf = 0;
            ...
            foreach ($array_temp as $catid_i) {
                if (isset($array_cat_admin[$admin_id][$catid_i])) {
                    if ($array_cat_admin[$admin_id][$catid_i]['admin'] == 1) {
                        ...
                        if ($array_cat_admin[$admin_id][$catid_i]['export_pdf_content'] == 1) {
                            ++$check_export_pdf;
                        } elseif (($status == 0 or $status == 4 or $status == 5) and $post_id == $admin_id) {
                            ++$check_export_pdf;
                            $_permission_action['waiting'] = true;
            ...
            if ($check_export_pdf == sizeof($array_temp)) {
                $check_permission_export_pdf = true;
                //here
            }
        }
        ...
        if ($check_permission_export_pdf) {
            $admin_funcs['export_pdf'] = nv_link_export_pdf_page($id);
            $_permission_action['export_pdf'] = true;
        }
        $data[$id] = [
            'id' => $id,
