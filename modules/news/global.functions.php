...
/**
 * nv_link_export_pdf_page()
 *
 * @param int $id
 * @return string
 */

function nv_link_export_pdf_page($id)
{
    global $db, $module_name, $module_data, $title, $lang_module, $module_config;
    $title = '';
    $alias = '';
    $hometext = '';
    list($id, $title, $alias, $hometext) = $db->query('SELECT id, title, alias, hometext FROM ' . NV_PREFIXLANG . '_' . $module_data . '_rows WHERE id=' . (int) $id)->fetch(3);
    $link = '<a class="btn btn-danger btn-xs" href="javascript:void(0);" onclick="nv_export_pdf_content(' . $id . ',  \''.$title.'\',  \''.$alias.'\',  \''.$hometext.'\')"> PDF </a>';
    // $link = '<a class="btn btn-success btn-xs" href="javascript:void(0);" onclick="echo "123"; "> PDF </a>';
    $link = '<a class="btn btn-primary btn-xs btn_edit" href="export.php?alias=\''.$alias.'\'&title=\''.$title.'\'&hometext=\''.$hometext.'\'"><em class="fa fa-edit margin-right"></em>PDF</a>';
    return $link;
}
...
<!-- <script type="text/javascript">
    function nv_export_pdf_content($id, $title, $alias, $hometext){
        code = <?php 
            // require_once('../../dompdf/autoload.inc.php');
            // use Dompdf\Dompdf;
            // $dompdf = new Dompdf();
            // $html = `<h1 style="color:blue;">helloo</h1>`
            ?>
        return code;
    }
</script> -->